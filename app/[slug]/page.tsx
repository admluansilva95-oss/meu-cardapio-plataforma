...
export default function PublicCardapioPage() {
  ...
  if (!slug) {
    return (
      <div className={`${shellClass} flex items-center justify-center px-6`}>
        <p className="text-sm text-[#6e6e73]">URL inválida.</p>
      </div>
    );
  }
  ...
  if (!restaurante) {
    return (
      <div className={`${shellClass} px-6 py-24`}>
        <div className="mx-auto max-w-lg text-center">
          <h1 className="text-2xl font-semibold tracking-tight text-[#1d1d1f]">Restaurante não encontrado</h1>
          <p className="mt-4 text-sm leading-relaxed text-[#6e6e73]">
            Não há cardápio cadastrado para <span className="font-mono text-[#424245]">/{slug}</span>.
            Confira o link ou entre em contato com o estabelecimento.
          </p>
          <Link
            href="/"
            className="mt-10 inline-flex rounded-xl border border-black/[0.08] bg-white px-5 py-2.5 text-sm font-semibold text-[#1d1d1f] shadow-sm transition hover:bg-[#fafafa]"
          >
            Voltar ao início
          </Link>
        </div>
  ...
  return (
    <div className={shellClass}>
        ...
        <div className="mx-auto flex max-w-6xl flex-col gap-10 px-5 py-12 sm:flex-row sm:items-end sm:justify-between sm:px-8 sm:py-16">
          <div className="flex items-start gap-6 sm:items-center">
            <div
              ...
              style={{ boxShadow: `0 0 0 1px color-mix(in srgb, ${accent} 22%, transparent), 0 18px 50px -28px rgba(0,0,0,0.35)` }}
            >
              {restaurante.logo ? (
      ...
      {/* Carrinho flutuante — abre drawer premium antes do WhatsApp */}
      <div className="pointer-events-none fixed inset-x-0 bottom-6 z-30 flex justify-end px-5 sm:bottom-8 sm:px-8">
        <div className="pointer-events-auto flex flex-col items-end gap-3">
          ...
          <button
            ...
            style={{ boxShadow: `0 18px 50px -18px color-mix(in srgb, ${accent} 45%, rgba(0,0,0,0.5))` }}
