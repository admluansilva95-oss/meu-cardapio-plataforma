...
import { wrapFetchEnsureSupabaseWireSafe } from "@/lib/supabase/normalize-supabase-browser-fetch";
 ...
 * O `fetch` é ainda envolvido por `wrapFetchEnsureSupabaseWireSafe` para corrigir `Content-Type`
 * em escritas JSON no PostgREST (`/rest/v1`).
...
export function createBrowserSupabaseClient() {
  return createBrowserClient(getPublicSupabaseProjectUrl(), process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!, {
    global: { fetch: wrapFetchEnsureSupabaseWireSafe(getNativeFetchForSupabase()) },
